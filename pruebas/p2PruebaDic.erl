...
descomprimirData(L,Arb,SymbolsDic,Filename) -> 
							{ok, Fd} = file:open(Filename, [append, delayed_write]),
							descomprimirData(L,Arb,SymbolsDic,[],Fd),
							file:close(Fd),
							ok.

descomprimirData([],Arb,SymbolsDic,Acum,Fd) ->
		io:format("FINNNNN ~n",[]),
	Final = lists:flatten(interpretarFinal(Arb,Acum)),
	writeListDescomprimir(Final,Fd);

descomprimirData([H|T],Arb,SymbolsDic,Acum,Fd) -> 
		io:format("Tam: ~p~n",[length([H|T])]),
		TotalAcum = Acum ++ completeByteRev(dec2bin(H)),
		Val = lists:concat(TotalAcum),
		Caseval = dict:find(Val,SymbolsDic),
		case Caseval of
			error ->
				descomprimirData(T,Arb,SymbolsDic,TotalAcum,Fd);
			_Else ->
				Char = dict:fetch(Val, SymbolsDic),
				file:write(Fd, [Char]),
				descomprimirData(T,Arb,SymbolsDic,[],Fd)
		end.




%descomprimirData([],SymbolsDic,Acum,_Size,Fd) ->
%	Final = lists:flatten(interpretarFinal(Arb,Acum)),
%	writeListDescomprimir(Final,Fd);

%descomprimirData([H|T],SymbolsDic,Acum,Size,Fd) when Size >= CL ->
%	io:format("Tam: ~p~n",[length([H|T])]),
%	TotalAcum = Acum ++ completeByteRev(dec2bin(H)),
%	{Lelem,Lrestante} = interpretar(SymbolsDic,TotalAcum),
%	writeListDescomprimir(Lelem,Fd),
%	descomprimirData(T,SymbolsDic,Lrestante,length(Lrestante),Fd);

%descomprimirData([H|T],SymbolsDic,Acum,Size,Fd) -> 
%		io:format("Tam: ~p~n",[length([H|T])]),
%	TotalAcum = Acum ++ completeByteRev(dec2bin(H)),
%	descomprimirData(T,SymbolsDic,TotalAcum,length(TotalAcum),Fd).
...
convertSymbolsToDic(L) -> convertSymbolsToDic(L,[]).
convertSymbolsToDic([],Acum) -> dict:from_list(Acum);
convertSymbolsToDic([{Char,Bins}|T],Acum) -> convertSymbolsToDic(T,Acum ++ [{lists:concat(Bins),Char}]).
...
decompress(Filename,Newfile,Symbolpath)-> 
						...
						SymbolsDic = convertSymbolsToDic(Symbol),
						io:format("Simbols dic listo ~n",[]),
						descomprimirData(ListaCompresion,ArbolHuffman,SymbolsDic,Newfile).
...
% Prueba:
% HuffmanServer = spawn (fun()->p2:huffmanServer(0,[],"pablo.huff",7,[])end).
...
% p2:decompress("biblia.huff", "b.txt",  "biblia.huff.simbolos").
% p2:decompress("mama.huff", "mama.txt",  "mama.huff.simbolos").