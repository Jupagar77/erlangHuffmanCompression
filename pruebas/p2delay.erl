...
toCode([],_Dict,Acum,_Size,Listafinal,Filename,Fd)-> Final = group8(Acum), 
										 FinalDec = listToDec(Final),
										 writeList(FinalDec,Filename,Fd);

toCode([H|T],Dict,Acum,Size,Listafinal,Filename,Fd) when Size >= 8 ->
...
% Prueba:
% HuffmanServer = spawn (fun()->p2delay:huffmanServer(0,[],"pablo.mp4.huff",7,[])end).
% p2delay:hiloComprimir("pablo00", HuffmanServer).
